        ...
        def acts_as_category(params = {})
          ...
          named_scope :permitted, lambda {
            if permissions.empty?
              { :conditions => "#{hidden_column} IS NULL OR #{hidden_column}=CAST(0 AS boolean)", :order => order_by }
            else
              { :conditions => ["#{hidden_column} IS NULL OR #{hidden_column}=CAST(0 AS boolean) OR id IN (?)", class_variable_get(:@@permissions)], :order => order_by }
            end
          }
